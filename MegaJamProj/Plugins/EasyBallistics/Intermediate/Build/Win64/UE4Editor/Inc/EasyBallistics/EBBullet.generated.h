...
#define HostProject_Plugins_EasyBallistics_Source_EasyBallistics_Public_EBBullet_h_29_RPC_WRAPPERS \
	...
	virtual bool CollisionFilter_Implementation(FHitResult HitResult); \
	virtual float GetSpeedOfSound_Implementation(UWorld* World, FVector Location); \
	virtual float GetAirDensity_Implementation(UWorld* World, FVector Location); \
	virtual FVector GetWind_Implementation(UWorld* World, FVector Location); \
	virtual FVector UpdateVelocity_Implementation(UWorld* World, FVector Location, FVector PreviousVelocity, float DeltaTime); \
