...
float AEBBullet::Trace(FVector start, FVector PreviousVelocity, float delta, TEnumAsByte<ECollisionChannel> CollisionChannel) {
	...
	if (OwnerSafe) {
		CollisionParameters.AddIgnoredActors(SafeLaunchIgnoredActors);

		CollisionParameters.AddIgnoredActor(GetOwner());

		if (SafeLaunchIgnoreAttachParent && GetOwner()) {
			AActor* AttachedRoot=GetOwner();
			...
			while(true){ //find attachment root
				...
				if (AttachedTo) {
					CollisionParameters.AddIgnoredActor(AttachedTo);
					AttachedRoot = AttachedTo;
				}
				else break;
			}
			CollisionParameters.AddIgnoredActor(AttachedRoot);
			if(SafeLaunchIgnoreAllAttached) CollisionParameters.AddIgnoredActors(GetAttachedActorsRecursive(AttachedRoot));
		}
	}
	...
	if (Hit) {
		...
		EPenTraceType penType = EPenTraceType::PT_Back;
		...
		if (MaterialResponseMap != nullptr) {
			...
			if (ResponseEntry != nullptr) {
				...
				penType = ResponseEntry->PenTraceType;
		...
		if (PenetrationDistance > 0.0f) {
			if (!neverPenetrate) {
				switch (penType) {
				case EPenTraceType::PT_Back:
					block = penTrace(HitResult.Location - (HitResult.Normal * CollisionMargin), HitResult.Location + PenetrationVector * PenetrationDistance, HitResult.Component, false, false, CollisionChannel, exitLoc, exitNormal);
					break;
				case EPenTraceType::PT_Solid:
					block = penTrace(HitResult.Location - (HitResult.Normal * CollisionMargin), HitResult.Location + PenetrationVector * PenetrationDistance, HitResult.Component, true, false, CollisionChannel, exitLoc, exitNormal);
					break;
				case EPenTraceType::PT_Forward:
					block = penTrace(HitResult.Location - (HitResult.Normal * CollisionMargin), HitResult.Location + PenetrationVector * PenetrationDistance, HitResult.Component, true, true, CollisionChannel, exitLoc, exitNormal);
					break;
				}
			}
		}
		...
		if (block >= 0.999999f) {
			...
			//penetration
			float RemainingEnergy = FMath::Pow(1.0f - block, 2.0f);
		...
		//impact actual
		if (HasAuthority()) {
			OnImpact(Ricochet, Penetration, HitResult.Location, Velocity, HitResult.Normal, GetActorLocation(), NewVelocity, Impulse, PenetrationDepth, HitResult.Actor.Get(), HitResult.Component.Get(), HitResult.BoneName, HitResult.PhysMaterial.Get(), HitResult);
		}
...
TArray<AActor*> AEBBullet::GetAttachedActorsRecursive(AActor* Actor, uint16 Depth) {
