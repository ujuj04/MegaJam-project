	...
	if (BulletClass != nullptr && BulletOwner != nullptr) {
		...
		if (!Default->Shotgun) {
			...
			for (int i = 0; i < Default->ShotCount; i++) {
				...
				FVector BVelocity = Default->RandomStream.VRandCone(BulletVelocity, Default->ShotSpread)*Vel;
				AEBBullet* bullet = SpawnOrReactivate(BulletOwner->GetWorld(), BulletClass, Transform, BVelocity, BulletOwner, BulletInstigator);
			}
		}
	}
	else {
	...
	if (BulletClass != nullptr && BulletOwner != nullptr) {
		...
		if (!Default->Shotgun) {
			...
			for (int i = 0; i < Default->ShotCount; i++) {
				...
				FVector BVelocity = Default->RandomStream.VRandCone(BulletVelocityNew, Default->ShotSpread)*Vel;
				AEBBullet* bullet = SpawnOrReactivate(BulletOwner->GetWorld(), BulletClass, Transform, BulletVelocity, BulletOwner, BulletInstigator);
			}
		}
	}
