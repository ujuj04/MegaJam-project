...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Weapon") float Spread=0.0f;
	...
	UFUNCTION(BlueprintPure, Category = "Ammo") int GetAmmoCount(bool CountChambered);
	UFUNCTION(BlueprintPure, Category = "Ammo") TArray<TSubclassOf<class AEBBullet>> GetAmmo(bool CountChambered);
	...
	UFUNCTION(BlueprintCallable, Category = "Prediction") void PredictHit(bool &Hit, FVector& HitLocation, float& HitTime, AActor*& HitActor, TArray<FVector>& Trajectory, TSubclassOf<class AEBBullet> BulletClass, FVector StartLocation, FVector PredictAim, float MaxTime = 10.0f , float Step = 0.1f, bool GetFullTrajectory=false);
...
private:
	...
	bool PredTrace(UWorld* World, AEBBullet* Bullet, FVector Start, FVector End, FVector &HitLocation, float &Time, AActor* &Actor);
};