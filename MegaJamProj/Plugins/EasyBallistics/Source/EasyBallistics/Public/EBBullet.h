...
public:	
	...
	UFUNCTION(BlueprintNativeEvent, Category = "EBBullet|Flight")FVector UpdateVelocity(UWorld* World, FVector Location, FVector PreviousVelocity, float DeltaTime);
	UFUNCTION(BlueprintNativeEvent, Category = "EBBullet|World") FVector GetWind(UWorld* World, FVector Location);
	UFUNCTION(BlueprintNativeEvent, Category = "EBBullet|World") float GetAirDensity(UWorld* World, FVector Location);
	UFUNCTION(BlueprintNativeEvent, Category = "EBBullet|World") float GetSpeedOfSound(UWorld* World, FVector Location);
	UFUNCTION(BlueprintNativeEvent, Category = "EBBullet|World") bool CollisionFilter(FHitResult HitResult);
...
private:
	...
	TArray<AActor*> GetAttachedActorsRecursive(AActor* Actor,uint16 Depth=0);

	float penTrace(FVector start, FVector end, TWeakObjectPtr<UPrimitiveComponent,FWeakObjectPtr> comp, bool solid, bool reverse, TEnumAsByte<ECollisionChannel> channel, FVector &exitLoc, FVector &exitNormal);
	...
	FHitResult FilterHits(TArray<FHitResult> Results, bool &hit);
